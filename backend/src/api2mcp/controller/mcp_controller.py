...
async def _handle_jsonrpc_method(
    ...
    if method == "initialize":
        ...
        return await mcp_service.invoke_tool(full_tool_name, arguments, tool_id)
...
async def _resolve_project_with_version(
    ...
    async with async_session_maker() as session:
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Project not found: {tool_name}@{version}"
        )
...
async def mcp_project_jsonrpc_with_version(
    request: Request, tool_name: str, version: str
):
    """
