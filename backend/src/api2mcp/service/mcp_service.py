...
class MCPService:
    ...
    async def build_request_url(
        self, tool: Api2mcpTool, arguments: Dict[str, Any]
    ) -> str:
        """构建请求 URL（处理路径参数）"""
        if not tool.base_url or not tool.base_url.strip():
            raise ValueError(
                f"Tool '{tool.tool_name}@{tool.version}' base_url is not configured. "
                "Please configure the API base URL in the management page first"
            )

        # 确保 base_url 以 http:// 或 https:// 开头
        base_url = tool.base_url.strip()
            ...
            for tool in tools_from_db:
                param_result = await session.execute(
                    select(Api2mcpParameter).where(
                        Api2mcpParameter.tool_id == tool.id
                    )
                )
    ...
    async def invoke_tool(
        ...
        """调用工具"""
        # 解析工具名格式：tool_name@version 或 tool_name
        version = None
        if "@" in tool_name:
        ...
        async with async_session_maker() as session:
            ...
            if not tool:
                return {
                    "content": [{"type": "text", "text": f"Tool not found or not enabled: {tool_name}"}],
                    "isError": True,
                }
