...
class ParameterService:
    ...
    async def list_parameters(self, tool_id: str) -> List[Dict[str, Any]]:
        """获取工具的参数列表（树结构）"""
        async with async_session_maker() as session:
            param_repo = ParameterRepository(session)
    ...
    async def update_parameter(
        self, tool_id: str, param_id: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """更新参数"""
        async with async_session_maker() as session:
            param_repo = ParameterRepository(session)

            param = await param_repo.get_parameter_by_id(param_id, tool_id)
            if not param:
