...
class ParameterRepository(BaseRepository[Api2mcpParameter]):
    ...
    async def create_parameter(self, baseinfo_id: str, **kwargs) -> Api2mcpParameter:
        """创建参数"""
        if 'id' not in kwargs:
            kwargs['id'] = str(uuid4())
        kwargs['baseinfo_id'] = baseinfo_id
        return await self.create(**kwargs)

    async def get_parameters_by_baseinfo(self, baseinfo_id: str) -> List[Api2mcpParameter]:
        """获取基础信息的所有参数"""
        result = await self.session.execute(
            select(Api2mcpParameter)
            .where(Api2mcpParameter.baseinfo_id == baseinfo_id)
            .order_by(Api2mcpParameter.sort_order)
        )
        return list(result.scalars().all())

    async def get_parameter_by_id(
        self, param_id: str, baseinfo_id: str
    ...
    async def get_root_parameters(self, baseinfo_id: str) -> List[Api2mcpParameter]:
        """获取顶级参数（无父参数的参数）"""
        result = await self.session.execute(
            select(Api2mcpParameter)
            .where(Api2mcpParameter.baseinfo_id == baseinfo_id)
    ...
    async def delete_parameters_by_baseinfo(self, baseinfo_id: str) -> bool:
        """删除基础信息的所有参数"""
        await self.session.execute(
